...
int flag_arrow=0,flag_balloon=1,count_arrow=10,count_balloon=15;
...
getimage(p-4*BALLOON_SIZE,q-5*BALLOON_SIZE,p+4*BALLOON_SIZE,q+7*BALLOON_SIZE,balloon);
	...
	while (!finish)
	{
		...
		itoa(level,char_level,10); //converts int to string
		setfillstyle(0,0);
		...
		else
	{
		...
		if( kbhit() ) //checks if the key is being pressed
		{
		key = testkeys(); 
		if(key==77)
		flag_arrow = 1;
	...
	if( key == 77 || flag_arrow)
		...
		setfillstyle(0,0);
		bar(200,370,230,400);
		itoa(count_balloon,cnt_ball,10); //converts int to string
		outtextxy(200,380,cnt_ball);
		flag_balloon=0;
		score+=20;
		itoa(score,score1,10); //converts int to string
		setfillstyle(0,0);
	...
	outtextxy(50,430,"THIS GAME HAS BEEN DEVELOPED BY HARSH VARTAK,HRISHIKESH AVANOOR");
	outtextxy(50,450,"AND HIMANSHU SANKLECHA SECOND YEAR, COMPUTER ENGINEERING");
	while( getch() != 'q');
	finish=1;
	break;
		 }
	...
	*x+=ARROW_SIZE;
	...
	if (*x>590)
	{
		*x=155;
		...
		bar(200,400,220,425);
		itoa(count_arrow,cnt_arrow,10); //converts int to string
...
int testkeys() // it is a bareback call to the OS to see if the desired keys are being pressed
{
	union REGS ii, oo ;
	ii.h.ah = 0 ;
	int86 ( 22, &ii, &oo ) ;     /*allows to call interrupts in the program*/
...
void start()
{
	...
	outtextxy(10,230,"5. First level has 10 arrows and 15 balloons.");
	outtextxy(10,270,"6. You require to score 100 points to enter the second level.");
	outtextxy(10,310,"7. Second level has 10 arrows and 13 balloons.");
	/*outtextxy(10,350,"8. You require to score 200 points to enter the third level.");
	outtextxy(10,390,"9. Third level has 10 arrows and 12 balloons.");*/
	settextstyle(7,0,1);
